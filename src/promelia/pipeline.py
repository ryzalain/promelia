"""Promelia Pipeline Orchestrator.

Runs pipeline stages in sequence or concurrently (streaming mode).

Usage (via CLI):
    promelia run                        # all stages, sequential
    promelia run --stream               # all stages, concurrent
    promelia run discover enrich        # specific stages
    promelia run score tailor cover     # LLM-only stages
    promelia run --dry-run              # preview without executing
"""
...
from promelia.database import init_db, get_connection, get_stats
...
def _run_discover(workers: int = 1) -> dict:
    ...
    try:
        from promelia.discovery.jobspy import run_discovery
        run_discovery()
        stats["jobspy"] = "ok"
    ...
    try:
        from promelia.discovery.workday import run_workday_discovery
    ...
    try:
        from promelia.discovery.smartextract import run_smart_extract
...
def _run_enrich(workers: int = 1) -> dict:
    ...
    try:
        from promelia.enrichment.detail import run_enrichment
...
def _run_score() -> dict:
    ...
    try:
        from promelia.scoring.scorer import run_scoring
        run_scoring()
        return {"status": "ok"}
    except Exception as e:
...
def _run_pdf() -> dict:
    ...
    try:
        from promelia.scoring.pdf import batch_convert
        batch_convert()
        return {"status": "ok"}
    except Exception as e:
...
def run_pipeline(
    ...
    console.print(Panel.fit(
        f"[bold]Promelia Pipeline[/bold] ({mode})",
        border_style="blue",
    ))
