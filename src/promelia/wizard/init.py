"""Promelia first-time setup wizard.

Interactive flow that creates ~/.promelia/ with:
...
def _setup_profile() -> dict:
    ...
    console.print(Panel("[bold]Step 2: Profile[/bold]\nTell Promelia about yourself. This powers scoring, tailoring, and auto-fill."))
...
def _setup_ai_features() -> None:
    ...
    if not Confirm.ask("Enable AI scoring and resume tailoring?", default=True):
        console.print("[dim]Discovery-only mode. You can configure AI later with [bold]promelia init[/bold].[/dim]")
        return
    ...
    env_lines = ["# Promelia configuration", ""]
...
def _setup_auto_apply() -> None:
    ...
    console.print(Panel(
        ...
        "Promelia can autonomously fill and submit job applications\n"
        "using Claude Code as the browser agent."
    ))

    if not Confirm.ask("Enable autonomous job applications?", default=True):
        console.print("[dim]You can apply manually using the tailored resumes Promelia generates.[/dim]")
        return
    ...
    if Confirm.ask("Configure CapSolver API key? (optional)", default=False):
        ...
        if ENV_PATH.exists():
            ...
            ENV_PATH.write_text(f"# Promelia configuration\nCAPSOLVER_API_KEY={capsolver_key}\n", encoding="utf-8")
...
def run_wizard() -> None:
    ...
    console.print(
        Panel.fit(
            "[bold green]Promelia Setup Wizard[/bold green]\n\n"
            "This will create your configuration at:\n"
            f"  [cyan]{APP_DIR}[/cyan]\n\n"
            "You can re-run this anytime with [bold]promelia init[/bold].",
            border_style="green",
        )
    )
    ...
    from promelia.config import get_tier, TIER_LABELS, TIER_COMMANDS
    ...
    unlock_hint = ""
    if tier == 1:
        unlock_hint = "\n[dim]To unlock Tier 2: configure an LLM API key (re-run [bold]promelia init[/bold]).[/dim]"
    elif tier == 2:
