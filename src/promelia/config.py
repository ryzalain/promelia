"""Promelia configuration: paths, platform detection, user data."""
...
APP_DIR = Path(os.environ.get("PROMELIA_DIR", Path.home() / ".promelia"))

# Core paths
DB_PATH = APP_DIR / "promelia.db"
...
def load_profile() -> dict:
    """Load user profile from ~/.promelia/profile.json."""
    import json
    if not PROFILE_PATH.exists():
        raise FileNotFoundError(
            f"Profile not found at {PROFILE_PATH}. Run `promelia init` first."
        )
    return json.loads(PROFILE_PATH.read_text(encoding="utf-8"))


def load_search_config() -> dict:
    """Load search configuration from ~/.promelia/searches.yaml."""
    import yaml
...
def load_env():
    """Load environment variables from ~/.promelia/.env if it exists."""
...
def check_tier(required: int, feature: str) -> None:
    ...
    if required >= 2 and not any(os.environ.get(k) for k in ("GEMINI_API_KEY", "OPENAI_API_KEY", "LLM_URL")):
        missing.append("LLM API key — run [bold]promelia init[/bold] or set GEMINI_API_KEY")
    if required >= 3:
