...
parses salary ranges, and stores everything in the Promelia database.
...
from promelia.database import get_connection, init_db, store_jobs
...
def run_discovery(cfg: dict | None = None) -> dict:
    ...
    if not cfg:
        log.warning("No search configuration found. Run `promelia init` to create one.")
