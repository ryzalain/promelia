...
from promelia import config
from promelia.config import DB_PATH
from promelia.database import get_connection, init_db, ensure_columns
from promelia.llm import get_client
...
def set_proxy(proxy_str: str | None):
    ...
    if proxy_str:
        from promelia.discovery.jobspy import parse_proxy
...
def _load_base_urls() -> dict[str, str | None]:
    ...
    from promelia.config import load_base_urls
...
def extract_with_llm(page, url: str) -> dict:
    ...
    try:
        ...
        from promelia.discovery.smartextract import extract_json
