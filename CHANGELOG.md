...
All notable changes to Promelia will be documented in this file.
...
### Added
- **Parallel workers for discovery/enrichment** - `promelia run --workers N` enables
...
- **Apply utility modes** - `--gen` (generate prompt for manual debugging), `--mark-applied`,
  `--mark-failed`, `--reset-failed` flags on `promelia apply`
- **Dry-run mode** - `promelia apply --dry-run` fills forms without clicking Submit
...
- **Config YAML not found after install** - moved `config/` into the package at
  `src/promelia/config/` so YAML files (employers, sites, searches) ship with `pip install`
...
- Interactive setup wizard (`promelia init`)
